...
def send_telegram_message(message, btc_status_1h, btc_status_4h, is_new_coin=False, btc_price_change_percentage=0.0):
    ...
    for retry_count in range(1, max_retries + 1):
        try:
            # 메시지와 BTC 상태를 함께 보내기
            message_with_status = f"{message}\n비트-[5️⃣▪️2️⃣0️⃣]{' 🟩 LONG ' if btc_status_1h else ' 🟥 SHORT 나의 매매법은 비트하락추세에는 안통한다. 인정하자'}\n비트-[2️⃣0️⃣▪️5️⃣0️⃣]{' 🟩 LONG ' if btc_status_4h else ' 🟥 SHORT 나의 매매법은 비트하락추세에는 안통한다. 인정하자'}"
            if is_new_coin:
                message_with_status += ""
...
def send_golden_cross_message(golden_cross_coins, btc_status_1h, btc_status_4h, btc_price_change_percentage):
    ...
    golden_trade_price_result = {coin: trade_price for coin, trade_price in golden_trade_price_result.items() if trade_price >= 1}

    if not golden_trade_price_result:
        message = "🔴 현재 1억 이상의 거래대금을 가진 코인이 없습니다.\n\n업비트 상태 확인 완료."
        send_telegram_message(message, btc_status_1h, btc_status_4h)
        return

    message_lines = []
    message_lines.append("🎯 20-5 공통구간 출발자리만 하자")
    ...
    for coin, trade_price in sorted(golden_trade_price_result.items(), key=lambda x: x[1], reverse=True):
        ...
        # VWMA 상태 계산
        df = retry_request(pyupbit.get_ohlcv, coin, interval="minute240", count=200)
    ...
    if idx == 1:
        message = "🟢 현재 4시간봉 준비중인 코인이 없습니다. \n🟢 업비트 상태 확인 완료. \n"
        send_telegram_message(message, btc_status_1h, btc_status_4h)
        return

    message_lines.append("----------------------------------")
    message_lines.append("매매-[공통구간] 🅾️-✅️-🅾️ / 🅾️-✅️-✅️")
    message_lines.append("----------------------------------")
    final_message = "\n".join(message_lines)
