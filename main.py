...
def send_golden_cross_message(golden_cross_coins, btc_status_1h, btc_status_4h, btc_price_change_percentage):
    ...
    for idx, (coin, trade_price) in enumerate(golden_trade_price_result.items(), start=1):
        ...
        five_twenty = f"🟩({cnt_5_20})" if vwma_5 and vwma_20 and vwma_5 > vwma_20 else f"🅾️({cnt_5_20})"
        twenty_sixty = f"✅️({cnt_20_60})" if vwma_20 and vwma_60 and vwma_20 > vwma_60 else f"🟥({cnt_20_60})"
        sixty_hundredtwenty = f"🟩({cnt_60_120})" if vwma_60 and vwma_120 and vwma_60 > vwma_120 else f"🅾️({cnt_60_120})"

        message_lines.append(f"{idx}. {five_twenty}-{twenty_sixty}-{sixty_hundredtwenty}  {coin.replace('KRW-', '')} : {trade_price}억 ({price_change_str})")
